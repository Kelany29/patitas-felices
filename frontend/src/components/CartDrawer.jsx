  ...
  const handleCheckout = async () => {
    try {
      ...
      const responseMP = await axios.post(
        "https://patitas-felices-backend-wkn9.onrender.com/api/payments/create-preference",
        { items: cart },
            ...
            <div className="grow overflow-y-auto p-6 space-y-4 bg-gray-50/30">
              {cart.length === 0 ? (
                ...
                cart.map((item) => (
                  <div
                    key={item.id}
                    className="flex gap-4 items-center bg-white p-4 rounded-4xl border border-gray-100 shadow-sm"
                  >
                    <img
                ...
                <button
                  ...
                  className={`w-full py-5 rounded-4xl font-black text-lg shadow-xl transition-all flex items-center justify-center gap-3 active:scale-95 ${
                    isProcessing
                      ...
                      : "bg-[#009EE3] hover:bg-[#0086C3] text-white"
                  }`}
                >
                  {isProcessing ? (
